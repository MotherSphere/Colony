...
namespace
{
constexpr int kAddDialogRowHeight = ui::Scale(44);
...
void Application::RefreshAddAppDialogEntries()
{
    ...
    auto normalizedKey = [](const std::filesystem::path& path) {
    ...
    auto appendEntries = [&](const std::vector<RawEntry>& source) {
        for (const auto& raw : source)
        {
            ...
            std::string label = raw.path.filename().string();
            if (label.empty())
            {
    ...
    {
        addAppDialog_.errorMessage = "Directory is empty.";
    }
...
void Application::RenderAddAppDialog(double timeSeconds)
{
    ...
    const int panelPadding = ui::Scale(28);
    const int panelWidth = std::clamp(outputWidth - ui::Scale(240), ui::Scale(520), outputWidth - ui::Scale(80));
    const int panelHeight = std::clamp(outputHeight - ui::Scale(220), ui::Scale(420), outputHeight - ui::Scale(80));
    SDL_Rect panelRect{
    ...
    const int titleYOffset = ui::Scale(10);
    int cursorX = panelRect.x + panelPadding;
    int cursorY = panelRect.y + panelPadding + titleYOffset;
    ...
    const int parentButtonHeight = ui::Scale(38);
    ...
    cursorY += parentButtonHeight + ui::Scale(18);
    ...
    SDL_Rect listViewport{
        ...
        panelRect.h - cursorY - footerHeight};
    if (listViewport.h < ui::Scale(120))
    {
        listViewport.h = ui::Scale(120);
    }
...
bool Application::HandleAddAppDialogKey(SDL_Keycode key)
{
    ...
    case SDLK_ESCAPE:
        ...
        return true;
    case SDLK_BACKSPACE:
        if (addAppDialog_.parentAvailable)
        {
