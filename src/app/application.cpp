...
void Application::RefreshAddAppDialogEntries()
{
    ...
    if (addAppDialog_.entries.empty())
    {
        if (globalSearch)
        {
            addAppDialog_.errorMessage = hasFilter ? "No items match your search." : "Directory is empty.";
        }
        else
        {
            addAppDialog_.errorMessage = hasFilter && enumeratedAny ? "No items match your search." : "Directory is empty.";
        }
    }
