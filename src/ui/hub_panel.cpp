...
namespace
{
SDL_Color ResolveAccent(const HubPanelRenderer::BranchChrome& branch, const ThemeColors& theme)
{
    ...
    int activeBranchIndex) const
{
    ...
    SDL_Color accentColor = branches_.empty() ? theme.channelBadge : ResolveAccent(branches_.front(), theme);
    ...
    for (std::size_t index = 0; index < branches_.size(); ++index)
    {
        ...
        SDL_Color branchAccent = ResolveAccent(branch, theme);
