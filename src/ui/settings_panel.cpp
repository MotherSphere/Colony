    ...
    const std::unordered_map<std::string, bool>& toggleStates) const
{
    ...
    const int themeCardSpacing = Scale(12);
    const int themeColumns = availableWidth >= Scale(780) ? 3 : (availableWidth >= Scale(520) ? 2 : 1);
    ...
    const int themeCardHeight = Scale(80);
