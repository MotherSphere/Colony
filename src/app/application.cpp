...
const std::array<CustomThemeFieldDefinition, Application::CustomThemeDialogState::kColorFieldCount>
    kCustomThemeFields = {{{
...
void Application::RenderCustomThemeDialog(double timeSeconds)
{
    ...
    int panelHeight = std::min(outputHeight - ui::Scale(140), ui::Scale(640));
    panelHeight = std::max(panelHeight, ui::Scale(520));
    ...
    std::array<int, columns> columnCursors;
    columnCursors.fill(cursorY);
    ...
    for (std::size_t index = 0; index < kCustomThemeFields.size(); ++index)
    {
        ...
        int fieldY = columnCursors[column];
        ...
        {
            SDL_Rect labelRect{fieldX, fieldY, labelTexture.width, labelTexture.height};
            RenderTexture(renderer, labelTexture, labelRect);
            fieldY += labelRect.h + ui::Scale(6);
        }

        SDL_Rect fieldRect{fieldX, fieldY, columnWidth, fieldHeight};
        customThemeDialog_.colorFieldRects[index] = fieldRect;
        ...
        if (colorFocused)
        {
            ...
            if (caretVisible)
            {
                const int caretOffset = hasValue ? valueTexture.width : 0;
                const int caretX = textClip.x + caretOffset + ui::Scale(2);
                SDL_Rect caretClip{
                    textClip.x,
                    textClip.y + ui::Scale(6),
                    textClip.w,
                    textClip.h - ui::Scale(12)};
                SDL_RenderSetClipRect(renderer, &caretClip);
                SDL_SetRenderDrawColor(renderer, theme_.heroTitle.r, theme_.heroTitle.g, theme_.heroTitle.b, theme_.heroTitle.a);
                SDL_RenderDrawLine(
                    renderer,
                    ...
                    caretX,
                    fieldRect.y + fieldRect.h - ui::Scale(6));
                SDL_RenderSetClipRect(renderer, nullptr);
            }
        }

        columnCursors[column] = fieldRect.y + fieldRect.h + ui::Scale(20);
    }

    int fieldsBottom = columnCursors.front();
    for (int value : columnCursors)
    {
        fieldsBottom = std::max(fieldsBottom, value);
    }
    cursorY = fieldsBottom;
