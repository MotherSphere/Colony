...
const std::vector<AddDialogFileTypeFilter>& GetAddDialogFileTypeFilters()
{
    static const std::vector<AddDialogFileTypeFilter> kFilters = [] {
        auto makeFilter = [](const char* label, const char* extension) {
            ...
            return AddDialogFileTypeFilter{label, std::move(normalized)};
        };

        std::vector<AddDialogFileTypeFilter> filters;
        filters.emplace_back(AddDialogFileTypeFilter{"All files (*.*)", {}});
    ...
    if (optionWidth > 0)
    {
        ...
        if (addAppDialog_.filterDropdownOpen)
        {
            ...
            if (optionCount > 0)
            {
                ...
                {
                    SDL_Rect rowRect{dropdownRect.x, rowTop, dropdownRect.w, dropdownOptionHeight};
                    addAppDialog_.filterDropdownOptionRects[index] = rowRect;
