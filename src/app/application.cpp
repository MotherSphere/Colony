...
const std::vector<AddDialogFileTypeFilter>& GetAddDialogFileTypeFilters()
{
    static const std::vector<AddDialogFileTypeFilter> kFilters = [] {
        auto normalizeExtensions = [](std::initializer_list<const char*> extensions) {
        ...
        filters.push_back({"Executables (*.exe; *.bat; *.cmd)", normalizeExtensions({".exe", ".bat", ".cmd"})});
        filters.push_back({
            "Unix executables (*.sh; *.run; *.bin; *.AppImage)",
            normalizeExtensions({".sh", ".run", ".bin", ".appimage"}),
        });
        filters.push_back({"Mac applications (*.app; *.pkg)", normalizeExtensions({".app", ".pkg"})});
        filters.push_back({"Dynamic libraries (*.dll; *.so; *.dylib)", normalizeExtensions({".dll", ".so", ".dylib"})});
        return filters;
    }();
    return kFilters;
}
...
void Application::RefreshAddAppDialogEntries()
{
    ...
    if (globalSearch && hasFilter)
    {
        ...
        for (; iterator != end && (directories.size() + files.size()) < kMaxResults; iterator.increment(iteratorError))
        {
            ...
            entry.isDirectory = iterator->is_directory(statusError);
            if (statusError)
            {
    ...
    const bool fileTypeFilterActive = !fileFilters[fileFilterIndex].extensions.empty();

    auto passesFileTypeFilter = [&](const RawEntry& raw) {
        if (!fileTypeFilterActive || raw.isDirectory)
        {
            return true;
        }
        ...
        for (const auto& allowed : fileFilters[fileFilterIndex].extensions)
        {
...
void Application::RenderAddAppDialog(double timeSeconds)
{
    ...
    const bool fileFilterActive = !fileFilters[fileFilterIndex].extensions.empty();
    ...
    if (optionWidth > 0)
    {
        ...
        filterLabel.append(fileFilters[fileFilterIndex].label);
        renderOptionButton(addAppDialog_.filterButtonRect, filterLabel, fileFilterActive);
...
bool Application::HandleAddAppDialogMouseClick(int x, int y)
{
    ...
    if (addAppDialog_.filterButtonRect.w > 0 && addAppDialog_.filterButtonRect.h > 0
        && PointInRect(addAppDialog_.filterButtonRect, x, y))
    {
        const auto& fileFilters = GetAddDialogFileTypeFilters();
        const int filterCount = static_cast<int>(fileFilters.size());
        if (filterCount > 0)
        {
            int previous = addAppDialog_.fileTypeFilterIndex;
            if (cycleBackward)
            {
                addAppDialog_.fileTypeFilterIndex = (addAppDialog_.fileTypeFilterIndex - 1 + filterCount) % filterCount;
            }
            else
            {
                addAppDialog_.fileTypeFilterIndex = (addAppDialog_.fileTypeFilterIndex + 1) % filterCount;
            }
