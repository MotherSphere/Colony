...
const std::vector<AddDialogFileTypeFilter>& GetAddDialogFileTypeFilters()
{
    static const std::vector<AddDialogFileTypeFilter> kFilters = [] {
        auto makeFilter = [](const char* label, const char* extension) {
            ...
            return AddDialogFileTypeFilter{label, std::move(normalized), false, false, false};
        };

        std::vector<AddDialogFileTypeFilter> filters;
        filters.emplace_back(AddDialogFileTypeFilter{"All files (*.*)", {}, false, false, false});
...
#else
        AddDialogFileTypeFilter executableFilter{"Executable files", {}, false, false, true};
...
void Application::RefreshAddAppDialogEntries()
{
    ...
    if (globalSearch && hasFilter)
    {
        ...
        for (; iterator != end && (directories.size() + files.size()) < kMaxResults; iterator.increment(iteratorError))
        {
            ...
            const std::filesystem::file_status status = iterator->status(statusError);
            if (!statusError)
            {
                ...
                if (!entry.isDirectory)
                {
                    ...
                    const auto executableMask = std::filesystem::perms::owner_exec | std::filesystem::perms::group_exec | std::filesystem::perms::others_exec;
                    entry.isExecutable = (permissions & executableMask) != std::filesystem::perms::none;
#endif
                }
            }
