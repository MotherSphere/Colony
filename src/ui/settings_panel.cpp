    ...
    const std::unordered_map<std::string, float>& customizationValues) const
{
    ...
    const auto drawSectionHeader = [&](
                                        const colony::TextTexture& title,
                                        const colony::TextTexture& subtitle,
                                        std::string_view id,
                                        bool expanded,
                                        bool interactive) {
        ...
        SDL_Rect headerRect{bounds.x + horizontalPadding, cursorYStart, availableWidth, cursorY - cursorYStart};
        if (headerRect.h < Scale(36))
        {
            const int padding = Scale(36) - headerRect.h;
            headerRect.h += padding;
            cursorY += padding;
        }

        SDL_Rect drawHeaderRect = offsetRect(headerRect);
        if (interactive)
        {
            addInteractiveRegion(std::string{id}, RenderResult::InteractionType::SectionToggle, drawHeaderRect);

            const int chevronSize = Scale(12);
            const int chevronMargin = Scale(18);
            SDL_Point chevronCenter{
                drawHeaderRect.x + drawHeaderRect.w - chevronMargin,
                drawHeaderRect.y + drawHeaderRect.h / 2};
    ...
    if (sectionStates.appearanceExpanded)
    {
        drawSectionHeader(
            appearanceTitle_,
            appearanceSubtitle_,
            kAppearanceSectionId,
        ...
        const int rowContainerHeight = themeOptions_.empty()
            ? 0
            : static_cast<int>(themeOptions_.size()) * themeRowHeight
                + std::max<int>(0, (static_cast<int>(themeOptions_.size()) - 1)) * themeRowSpacing + listPadding * 2;
        ...
        {
            const ThemeOption& option = themeOptions_[index];
            ...
            colony::drawing::RenderFilledRoundedRect(renderer, drawOptionRect, 14, kRightRoundedCorners);
            SDL_SetRenderDrawColor(renderer, optionBorder.r, optionBorder.g, optionBorder.b, optionBorder.a);
            colony::drawing::RenderRoundedRect(renderer, drawOptionRect, 14, kRightRoundedCorners);
        ...
        const int addButtonHeight = Scale(72);
        ...
        int buttonTextY = addButtonRect.y + Scale(16);
        if (addThemeButtonLabel_.texture)
        {
            ...
            buttonTextY += addThemeButtonLabel_.height + Scale(8);
        }
    ...
    if (sectionStates.languageExpanded)
    {
        drawSectionHeader(
            languageTitle_,
            languageSubtitle_,
            kLanguageSectionId,
            sectionStates.languageExpanded,
            false);
    ...
    if (sectionStates.generalExpanded)
    {
        cursorY += Scale(10);
        drawSectionHeader(
            generalTitle_,
